...
def create_real_editable_table(data_list, container, label=None):
  ...
  # Create M3 Card with horizontal overflow for wide tables
  scroll_wrapper = ColumnPanel(
    role="outlined-card"
  )
  # Set width to force horizontal scrolling
  scroll_wrapper.width = "100%"

  table_container = FlowPanel(
    spacing="tight",
  )

  # Create header row - single row with all headers
  header_row = FlowPanel(spacing="tight")
  header_row.background = "#f5f5f5"

  for key in all_keys:
    header_label = Label(
      text=key.replace('_', ' ').title(),
      bold=True,
      width="250px",  # Much wider columns
      align="center",
      background="#f5f5f5",
      foreground="#333"
    )
